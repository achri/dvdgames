...
function dvd_show_photo() {
	...
	$.ajax({
		url: 'admin.php/<?php echo $link_controller?>/show_photo/'+target+'/dvd',
		type: 'POST',
...
function set_kode(kat_id) {
	...
	if (kat_id) {
		$.ajax({
			url: 'admin.php/<?php echo $link_controller?>/set_kode/'+kat_id+'/'+dvd_id,
			type: 'POST',
...
function tambah_dvd(status) {
	...
	if (status == "Tambah"){
		...
		if (validasi("form#dvd_form")) {
			$.ajax({
				url: 'admin.php/<?php echo $link_controller?>/tambah_dvd',
				type: 'POST',
...
function ubah_dvd(status) {
	...
	if (status == "Edit"){
		...
		if (validasi("form#dvd_form")) {
			$.ajax({
				url: 'admin.php/<?php echo $link_controller?>/ubah_dvd',
				type: 'POST',
				data: $('form#dvd_form').formSerialize(),
				success: function(data) {
					if (data) {
...
function hapus_dvd() {
	var dvd_id = $('#dvd_id').val(),
		$dlg_btn = {
			"HAPUS" : function() {
				$.ajax({
					url: 'admin.php/<?php echo $link_controller?>/hapus_dvd/'+dvd_id,
					type: 'POST',
					success: function(data) {
	...
	if (dvd_id) {
		konfirmasi("Data DVD ini akan dihapus ???",$dlg_btn);
	} 
	return false;
}
	...
	var button = $('.dvd_photos'), interval,
		dvd_upload = new AjaxUpload('#dvd_getgambar',{
			...
			onComplete: function(file, response){
				...
				$('#dvd_photos').load('admin.php/<?php echo $link_controller?>/show_photo/'+response);
				return false;
			}
		});
