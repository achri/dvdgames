...
function tambah_kat(status) {
	...
	if (status == "Tambah"){
		...
		if (validasi("form#kat_form")) {
			$.ajax({
				url: 'admin.php/<?php echo $link_controller?>/tambah_kategori',
				type: 'POST',
...
function ubah_kat(status) {
	...
	if (status == "Edit"){
		...
		if (validasi("form#kat_form")) {
			$.ajax({
				url: 'admin.php/<?php echo $link_controller?>/ubah_kategori',
				type: 'POST',
				data: $('form#kat_form').formSerialize(),
				success: function(data) {
					if (data) {
...
function hapus_kat() {
	var kat_id = $('#kat_id').val(),
		$dlg_btn = {
			"HAPUS" : function() {
				$.ajax({
					url: 'admin.php/<?php echo $link_controller?>/hapus_kategori/'+kat_id,
					type: 'POST',
					success: function(data) {
	...
	if (kat_id) {
		konfirmasi("Data Kategori ini akan dihapus ???",$dlg_btn);
	} 
	return false;
}
	...
	var button = $('.photos'), interval,
		upload = new AjaxUpload('#getgambar',{
			...
			onComplete: function(file, response){
				...
				this.enable();
				$('#gambar').val(response);	
				$('#photos').load('admin.php/<?php echo $link_controller?>/show_photo/'+response);
				return false;
			}
		});
});
