...
class Tbl_penjualan extends CI_Model
{
	...
	function set_trace_no(){
		$str_now = date("Y-m-d");
		$this->db->select_max('jual_trace','trace_no');
		$this->db->like('jual_tgl',$str_now,'after');
		$get_trace = $this->db->get('penjualan')->row();
		
		$trace_no = $get_trace->trace_no;
		$trace_no++;
		
		$return['trace_no'] = $trace_no;
		$return['trace_str'] = '0.'.str_pad($trace_no, 2, "0", STR_PAD_LEFT);
